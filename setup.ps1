...
if ($Cuda) {
    ...
    & $venvPython -m pip install -r environments\requirements_win_cuda.txt
} else {
    Write-Host '==> Installiere Abhängigkeiten (CPU)'
    & $venvPython -m pip install -r environments\requirements_win_cpu.txt
}
...
if (-not $NoModels) {
    ...
    if ($env:HF_TOKEN) {
        ...
        & $venvPython scripts\fetch_models.py --skip-whisper --pyannote
    } else {
